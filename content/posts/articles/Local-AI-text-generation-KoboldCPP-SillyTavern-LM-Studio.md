...
LM Studio отлично подходит для теста моделей, технических и простых вопросов, таких как код, ответы на вопросы, написание текста, рерайт и тд. и тп. В качестве бекенда LM Studio менее гибкий, чем KoboldCPP. И в качестве фронтенда не такой настраиваемый как SillyTavern, но минималистичный и приятный.
...
{{< callout/note >}}
Советы даны на KoboldCPP & llama.cpp, в другом ПО работа технологий может отличаться.
{{< /callout/note >}}
...
При использовании LLM важно, чтобы ключевые веса модели оставались в быстрой памяти (VRAM или RAM). MLOCK фиксирует данные в RAM, если часть модели загружена туда, предотвращая свопинг[^swapping] и снижая задержки.

[^Swapping]: Swapping - перемещение процессов или их частей из RAM на диск (файл подкачки).
...
В трансформерных моделях вычисление внимания обычно требует квадратичного количества операций при увеличении длины последовательности. FlashAttention оптимизирует этот процесс, позволяя быстрее обрабатывать большие последовательности, что особенно важно при работе с высоконагруженными серверными приложениями ИИ, где каждая миллисекунда на счету.
...
Качаем исходный код с GitHub [репозитория](https://github.com/SillyTavern/SillyTavern?sl) вручную, либо использует git:
...
«Auto-connect to Last Server» - тоже полезная опция, чтобы при запуске SillyTavern, не нужно было вручную нажимать на «Connect», а оно само подключалось к последнему бэкенд серверу.
...
На странице оригинала обычно можно найти не только описание модели, а еще настройки, в некоторых случаях даже пресеты для ST.
...
Наверное самый известный, популярный и надежный человек который делает GGUF кванты это Bartowski.
...
KoboldCPP в сочетании с SillyTavern - это мощный инструмент для энтузиастов, которым важна гибкость, кастомизация и возможность детальной настройки. Это идеальный выбор для RP и экспериментов с моделями. Однако из-за большого количества параметров и тонких настроек это может потребовать времени.
