...
И так, у меня есть блог, как вы могли заметить. И однажды я подумал, было бы круто что-то писать/менять прямо с телефона, смотреть превью и пушить изменения. А не как обычно я это делал через приложения для заметок в качестве черновика и без превью.
...
У меня Android 15. Termux скачан из [F-Droid](https://f-droid.org/packages/com.termux?sl). Markdown редактор нужен который умеет работать с файлами, то есть не только внутреннее хранилище приложения. А так же легкий и который не будет особо мусорить своими файлами, то есть не Obsidian, а например [Zettel Notes](https://play.google.com/store/apps/details?id=org.eu.thedoc.zettelnotes?nt).
...
Чтобы все обновить. Далее выполним:
...
Дальше все завист от вашего "хочу". В случае моего "хочу" мне нужен go и hugo для генерации сайта (превью, live server) и git для клонирования/обновления репозитория сайта, поэтому именно этот сценарий я и кратко опишу далее.
...
{{< callout/note >}}
Не знаю как с другими сервисами, но в случае с GitHub, если вы хотите чтобы имя в комитах было другим вы должны указать другой email, который не привязан к аккаунту, можно и выдумать что-нибудь, но учтите такие комиты не будут учитываться в вашей статистике.

Если в конфиге имя будет любым другим, но указан email который привязан к аккаунта GitHub, то GitHub подцепит имя и аватарку юзера для отображения в комитах, статистика работать будет, но имя будет как у вас на GitHub.
...
Проверить можно командой `go version`. 
...
В один момент покажется что оно зависло, даже лог идти не будет, но это не так. Когда завершится, выполните команду:

```bash
ls ~/go/bin/
```

Если вы видите там "hugo", значит оно скомпилировалось.
